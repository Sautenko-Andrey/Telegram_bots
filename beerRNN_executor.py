...
class ExecutorRNN:
    '''Predicts item and return result to the bot'''
    ...
    __path = "../beerBot_DATA/all_beers"
    __texts_dir = os.listdir(__path)

    def prepearing_data(self) -> list:
    ...
    def index_convert_to_text(self, indeces_list) -> list:
    ...
    def identify_item(self, user_text) -> tuple:
        '''Identifies item and returns tuple of the main data'''
        ...
        prediction = int(np.argmax(result))  # Converting to int coz mongo doesn't allow numpy.int64
        # print(result, prediction, sep='\n')

        # Going to DB and get desired data
        product = self.__beer_collection.find_one({"id": prediction})
        # print(product)
        return product["name"], product["img"], product["prices"]
        ...
        #     return (
        #     self.beer_collection.find_one({"id": 179})["name"], self.beer_collection.find_one({"id": 179})["img"],
        #     self.beer_collection.find_one({"id": 179})["prices"])
        # elif np.argmax(result) == 180:
        #     return (
        #     self.beer_collection.find_one({"id": 180})["name"], self.beer_collection.find_one({"id": 180})["img"],
        #     self.beer_collection.find_one({"id": 180})["prices"])
        # elif np.argmax(result) == 181:
        #     return (
        #     self.beer_collection.find_one({"id": 181})["name"], self.beer_collection.find_one({"id": 181})["img"],
        #     self.beer_collection.find_one({"id": 181})["prices"])
        # elif np.argmax(result) == 182:
        #     return (
        #     self.beer_collection.find_one({"id": 182})["name"], self.beer_collection.find_one({"id": 182})["img"],
        #     self.beer_collection.find_one({"id": 182})["prices"])
        # elif np.argmax(result) == 183:
        #     return (
        #     self.beer_collection.find_one({"id": 183})["name"], self.beer_collection.find_one({"id": 183})["img"],
        #     self.beer_collection.find_one({"id": 183})["prices"])
        # elif np.argmax(result) == 184:
        #     return (
        #     self.beer_collection.find_one({"id": 184})["name"], self.beer_collection.find_one({"id": 184})["img"],
        #     self.beer_collection.find_one({"id": 184})["prices"])
        # else:
